...
define keepalived::virtual_server ( 
	...
	if $state == "MASTER" { #Export only when MASTER
		exec{"add-loopback-DSR-${virtual_ipaddress}":
			...
			tag => "keepalived-exported-dsr-config-${virtual_ipaddress}-${name}",
		}
		exec{"add-arp_announce-config-DSR-${virtual_ipaddress}-${name}":
		...
		exec{"add-arp_ignore-config-DSR-${virtual_ipaddress}-${name}":
